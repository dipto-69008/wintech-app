...
const List<String> _canonicalTransferZones = [
  'Cumilla',
  'Mymensingh',
  'Dhaka',
  'Feni',
...
String _resolveTransferZone(
  ...
  for (final zone in _canonicalTransferZones) {
    if (zone.toLowerCase() == value.toLowerCase()) return zone;
  }

  // Older cached profiles may still contain an Area/legacy branch label.
  // Resolve it through the ERP's territory response, but never expose that
  // Area as a transfer option.
  for (final branch in branchRows) {
    final matches = [
      ...
      branch['zoneName'],
      branch['zone'],
    ].whereType<Object>().map((item) => item.toString().trim());
    if (!matches.any((item) => item.toLowerCase() == value.toLowerCase())) {
      continue;
    }
    final zone = branch['zoneName']?.toString().trim() ?? '';
    for (final canonical in _canonicalTransferZones) {
      if (canonical.toLowerCase() == zone.toLowerCase()) return canonical;
    }
  }
...
class _TransferListTabState extends State<_TransferListTab>
  ...
  Future<void> _deleteTransfer(Map<String, dynamic> transfer) async {
    final status = (transfer['status'] ?? 'pending').toString().toLowerCase();
    if (status != 'pending') {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('Only pending transfers can be deleted',
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        title: Text('Delete pending transfer?',
            style: GoogleFonts.hindSiliguri(fontWeight: FontWeight.w700)),
        content: Text(
          '${transfer['fromZoneName'] ?? transfer['fromBranch'] ?? '—'} → '
          '${transfer['toZoneName'] ?? transfer['toBranch'] ?? '—'}',
          style: GoogleFonts.hindSiliguri(),
        ),
        actions: [
          TextButton(
          ...
          ElevatedButton(
            ...
            child: Text('Delete', style: GoogleFonts.hindSiliguri()),
          ),
        ],
      ),
    );
    ...
    try {
      await ApiService.deleteStockTransfer(id);
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('Transfer deleted', style: GoogleFonts.hindSiliguri()),
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(e is ApiException ? e.message : 'Could not delete transfer',
            style: GoogleFonts.hindSiliguri()),
        backgroundColor: AppTheme.error,
      ));
    }
  ...
  Widget build(BuildContext context) {
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        ...all.map((t) => _TransferCard(
              t: t,
              fmt: _fmt,
              isDark: isDark,
              onDelete: t['_queued'] == true ||
                      (t['status']?.toString() ?? 'pending') != 'pending'
                  ? null
                  : () => _deleteTransfer(t),
            )),
        ],
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          if (onDelete != null)
            IconButton(
              onPressed: onDelete,
              tooltip: 'Delete pending transfer',
...
class _NewTransferTabState extends State<_NewTransferTab> {
  ...
  // Stock transfers are strictly Zone → Zone. Areas/legacy branches must
  // never appear in these dropdowns.
  List<String> _branches = List<String>.from(_canonicalTransferZones);
  ...
  Future<void> _loadData() async {
    ...
    if (!mounted) return;
    final cachedZone = _resolveTransferZone(
      user?.zoneName.isNotEmpty == true ? user!.zoneName : user?.zela ?? '',
      const [],
    );
    setState(() {
      _user = user;
      // Never show an Area/legacy branch in a Zone-to-Zone transfer form.
      _fromBranch = cachedZone.isEmpty ? null : cachedZone;
    });
    ...
    try {
      ...
      final employeeBranch = _resolveTransferZone(
        _user?.zoneName.isNotEmpty == true ? _user!.zoneName : _user?.zela ?? '',
        branches,
      );
      setState(() {
        _erpConnected = true;
        if (prods.isNotEmpty) _products = prods;
        _branches = List<String>.from(_canonicalTransferZones);
        if (employeeBranch.isNotEmpty) _fromBranch = employeeBranch;
      });
    } catch (_) {
  ...
  Widget _branchDropdown(
    ...
    return DropdownButtonFormField<String>(
      ...
      items: _branches
          .where((b) => excludedValue == null || b.toLowerCase() != excludedValue.toLowerCase())
          .map((b) => DropdownMenuItem(value: b,

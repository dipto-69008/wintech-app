...
class LocalStorageService {
  ...
  static List<TutorialModel> _defaultTutorials() => [
        TutorialModel(
          id: 'tut-1',
          title: 'App Introduction — Get Started',
          description: 'A complete guide on how to use the Wintech Agro app.',
        ...
        TutorialModel(
          ...
          description: 'How to create a new order from the POS screen and submit it.',
          videoUrl: '',
          category: 'Order',
          durationMinutes: 6,
        ),
        TutorialModel(
          id: 'tut-3',
          title: 'Commission & Sales Report',
          description: 'How your commission is calculated and where to view your sales report.',
          videoUrl: '',
          category: 'Commission',
          durationMinutes: 7,
        ),
        TutorialModel(
          ...
          description: 'How to view your monthly target and tips for achieving it.',
          videoUrl: '',
          category: 'Target',
          durationMinutes: 5,
        ),
  ...
  static Future<List<UserModel>> getAllEmployees() async {
    return [
      ...
      UserModel(id: 'emp-21', name: 'Milon Sarkar', email: 'milon@gmail.com', phone: '01711111021', role: UserModel.roleTeamMember, zela: 'Chattogram', thana: 'Kalurghat', myReferralCode: 'TM021', teamId: 'team-ctg-a', teamName: 'Chattogram Team Alpha', totalSales: 430000, totalCommission: 6450, pendingCommission: 2600),
      ...
      UserModel(id: 'emp-08', name: 'Mosammat Rima', email: 'rima@gmail.com', phone: '01711111008', role: UserModel.roleTeamMember, zela: 'Rajshahi', thana: 'Shah Makhdum', myReferralCode: 'TM008', teamId: 'team-raj', teamName: 'Rajshahi Team', totalSales: 320000, totalCommission: 4800, pendingCommission: 2000),
      ...
      UserModel(id: 'emp-30', name: 'Lubna Ahmed', email: 'lubna@gmail.com', phone: '01711111030', role: UserModel.roleTeamMember, zela: 'Sylhet', thana: 'Biswanath', myReferralCode: 'TM030', teamId: 'team-syl', teamName: 'Sylhet Team', totalSales: 290000, totalCommission: 4350, pendingCommission: 1700),
      // ── Khulna ─────────────────────────────────────────────────────────
      ...
      UserModel(id: 'emp-35', name: 'Imran Khan', email: 'imran@gmail.com', phone: '01711111035', role: UserModel.roleTeamMember, zela: 'Mymensingh', thana: 'Bhaluka', myReferralCode: 'TM035', teamId: 'team-mym', teamName: 'Mymensingh Team', totalSales: 245000, totalCommission: 3675, pendingCommission: 1400),
      // ── Barishal ────────────────────────────────────────────────────────
      ...
      UserModel(id: 'emp-42', name: 'Fatema Tuz Zohra', email: 'fatema@gmail.com', phone: '01711111042', role: UserModel.roleTeamMember, zela: 'Cumilla', thana: 'Daudkandi', myReferralCode: 'TM042', teamId: 'team-cum', teamName: 'Cumilla Team', totalSales: 395000, totalCommission: 5925, pendingCommission: 2300),
  ...
  static Future<void> seedDemoData() async {
    ...
    // ── 8 Support Tickets ─────────────────────────────────────────────────
    final tickets = [
      {'id': 'TKT-001', 'name': 'Karim Hossain', 'email': 'karim@gmail.com', 'category': 'Order Issue', 'problem': 'Not receiving confirmation after submitting an order. The order is also not showing in the order list.', 'status': 'Resolved', 'createdAt': now.subtract(const Duration(days: 50)).toIso8601String()},
      {'id': 'TKT-002', 'name': 'Salma Begum', 'email': 'salma@gmail.com', 'category': 'Payment Issue', 'problem': "Last month's payment status is not updating. It is not showing in the payment history.", 'status': 'In Progress', 'createdAt': now.subtract(const Duration(days: 40)).toIso8601String()},
      {'id': 'TKT-003', 'name': 'Nasrin Akter', 'email': 'nasrin@gmail.com', 'category': 'Login Issue', 'problem': 'Forgot my password. Not receiving the reset email.', 'status': 'Resolved', 'createdAt': now.subtract(const Duration(days: 35)).toIso8601String()},
      {'id': 'TKT-004', 'name': 'Jamal Ahmed', 'email': 'jamal@gmail.com', 'category': 'Product Issue', 'problem': 'After updating the product stock, the old quantity is still showing.', 'status': 'Pending', 'createdAt': now.subtract(const Duration(days: 28)).toIso8601String()},
      ...
      {'id': 'TKT-006', 'name': 'Marzia Haq', 'email': 'marzia@gmail.com', 'category': 'Delivery Issue', 'problem': 'After delivering the order, the status still shows "Pending".', 'status': 'Pending', 'createdAt': now.subtract(const Duration(days: 14)).toIso8601String()},
      {'id': 'TKT-007', 'name': 'Shamim Reza', 'email': 'shamim@gmail.com', 'category': 'Payment Issue', 'problem': 'After collecting bKash payment, not receiving the transaction ID.', 'status': 'Resolved', 'createdAt': now.subtract(const Duration(days: 9)).toIso8601String()},
      {'id': 'TKT-008', 'name': 'Tariq Mahmud', 'email': 'tariq@gmail.com', 'category': 'General Issue', 'problem': 'App is becoming slow. Loading many orders takes too long.', 'status': 'Pending', 'createdAt': now.subtract(const Duration(days: 3)).toIso8601String()},
    ];
  ...
  static Future<List<Map<String, dynamic>>> getCommissionHistory() async {
    ...
    if (raw == null) {
      ...
      return orders
          ...
          .map((o) => {
                'id': o.id,
                'source': '${o.customerName} — Order Delivery',
  ...
  // ── Stock Transfers ────────────────────────────────────────────────────
  static const _keyStockTransfers = 'wintech_stock_transfers';

  static Future<List<StockTransferModel>> getStockTransfers() async {
    final prefs = await SharedPreferences.getInstance();
    final raw = prefs.getString(_keyStockTransfers);
    if (raw == null) return [];
    final List<dynamic> list = jsonDecode(raw);
    return list
        .map((e) => StockTransferModel.fromMap(Map<String, dynamic>.from(e as Map)))
        .toList();
  }

  static Future<void> saveStockTransfer(StockTransferModel t) async {
    final list = await getStockTransfers();
    final idx = list.indexWhere((x) => x.id == t.id);
    if (idx >= 0) {
      list[idx] = t;
    } else {
    ...
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(
        _keyStockTransfers, jsonEncode(list.map((e) => e.toMap()).toList()));
  }
  ...
  static Future<void> deleteExpense(String id) async {
    ...
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(
        _keyExpenses, jsonEncode(list.map((e) => e.toMap()).toList()));
  }
  ...
  static Future<void> saveLeave(LeaveModel l) async {
    ...
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString(
        _keyLeaves, jsonEncode(list.map((e) => e.toMap()).toList()));
  }
  ...
  static List<OrderModel> _demoOrders() {
    ...
    return [
      OrderModel(
        ...
        srName: 'Karim Hossain (Officer)',
        customerId: 'demo-customer',
        customerName: 'Messrs Al-Amin Traders',
        items: [
          const OrderItem(
              productName: 'Product-A 1 kg',
              quantity: 20,
              unit: 'kg',
              unitPrice: 250),
          const OrderItem(
              productName: 'Product-B 500 g',
              quantity: 10,
              unit: 'Piece',
              unitPrice: 120),
        ],
      ...
      OrderModel(
        ...
        customerId: 'cust-002',
        customerName: 'New Dhaka Enterprise',
        items: [
          const OrderItem(
              productName: 'Product-C 2 Litre',
              quantity: 5,
              unit: 'Litre',
              unitPrice: 450),
          const OrderItem(
              productName: 'Product-D Box',
              quantity: 3,
              unit: 'Box',
              unitPrice: 1800),
        ],
      ...
      OrderModel(
        ...
        srName: 'Karim Hossain (Officer)',
        customerId: 'cust-003',
        customerName: 'Rahman Stores',
        items: [
          const OrderItem(
              productName: 'Product-E Dozen',
              quantity: 8,
              unit: 'Dozen',
              unitPrice: 360),
        ],
      ...
      OrderModel(
        id: 'ORD-DEMO-005',
        srId: 'demo-sr',
        srName: 'Sales Officer',
        customerId: 'cust-005',
        customerName: 'City Trading Company',
        items: [
          const OrderItem(
              productName: 'Product-B 500 g',
              quantity: 25,
